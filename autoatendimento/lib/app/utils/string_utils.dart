...
class StringUtils {
  ...
  static String padStart(String string, int minLength, String padChar) {
    ...
    String retorno = "";
    for (var i = string.length; i < minLength; i++) retorno += padChar;
  ...
  static String mascaraTelefoneCel(String cel) {
    if (cel.isEmpty || cel == "" || cel == null) {
      return "";
    }

    return cel.length == 10
        ? '(${cel.substring(0, 2)}) ${cel.substring(2, 6)}-${cel.substring(6, 10)}'
        : '(${cel.substring(0, 2)}) ${cel.substring(2, 7)}-${cel.substring(7, 11)}';
  }
        ...
        ni.subitens.forEach((subitem) {
          switch (subitem.tipo) {
            ...
            case "ADICIONAL":
              if (descricaoSub.isNotEmpty) descricaoSub += " | ";
              descricaoSub +=
                  '${subitem.descricao} +R\$ ${subitem.precoTotal!.toStringAsFixed(2)}';
              break;
            case "ITEM_COMBO":
              if (descricaoSub.isNotEmpty) descricaoSub += " | ";
              descricaoSub += "${subitem.descricao}";
