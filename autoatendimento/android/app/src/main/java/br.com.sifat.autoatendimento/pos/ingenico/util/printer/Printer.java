...
import br.com.sifat.autoatendimento.utils.StringUtils;
...
public class Printer {
     ...
     * Printer object.
     */
    private UPrinter printer = AutoAtendimentoApplication.getDeviceService().getPrinter();

    /**
     * Context.
     */
    private Context context = AutoAtendimentoApplication.getContext();
    ...
    public void addText(int align, String text) throws RemoteException {
        ...
        text = StringUtils.removeAcentuacao(text);

        int maxCaracteres = 48;

        //Tratativas para quebra de linha
        String[] split = text.split("\n");

        for (String linha : split) {

            //Se a linha, tem mais que 48 caracteres, vai fazer a tratativa por palavra
            if (linha.length() > maxCaracteres) {

                String[] splitEspaco = linha.split(" ");
                String textoLimitado = "";

                for (String palavra : splitEspaco) {

                    //Se o texto limitado for maior que 47, já tá pronto para impressão
                    if (textoLimitado.length() + palavra.length() > (maxCaracteres - 1)) {
                        printer.addText(align, textoLimitado);
                        textoLimitado = "";
                    }
                    textoLimitado = textoLimitado + " " + palavra;
                    textoLimitado = textoLimitado.trim();
                }

                //Imprime a palavra que sobrou, e caso ela for maior que 47, quebra a linha (usado em links)
                if (textoLimitado.length() > (maxCaracteres - 1)) {

                    while (textoLimitado.length() > (maxCaracteres - 1)) {
                        printer.addText(align, textoLimitado.substring(0, (maxCaracteres - 1)));
                        textoLimitado = textoLimitado.substring(47);
                    }
                    printer.addText(align, textoLimitado);
